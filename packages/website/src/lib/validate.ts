...
const validation = { rules: MATERIALX_VALIDATION_RULES, supportedCategories: supportedMaterialXCategories };
...
export const validateMaterialXText = (text: string): MaterialXValidationIssue[] =>
  validateDocument(parseMaterialX(text), validation);
...
export const analyzeMaterialXText = (path: string, text: string): MaterialXAnalysis => {
  try {
    ...
    return { issues: validateDocument(document, validation), summary: summarizeMaterialX(path, document) };
  } catch (error) {
